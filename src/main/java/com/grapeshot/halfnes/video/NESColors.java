...
public class NESColors {
	...
	public final static byte[][][] colbytes;

	private static final int[] paletteOrg = {
...
//	private static final int[] paletteOrg = {0x757575, 0x271B8F, 0x0000AB,
...
//	};
	...
	static {
		col = makeColors(paletteOrg);
		colbytes = colorsToBytes(col);
	}

	private static int[][] makeColors(int[] palette)
	{
		...
		int[][] colors = new int[8][palette.length];
		for (int i = 0; i < palette.length; i++) {
			int r = r(palette[i]);
			int b = b(palette[i]);
			int g = g(palette[i]);

			colors[0][i] = palette[i] |= 0xff000000;
			//emphasize red
			colors[1][i] = rgb(r, g * att, b * att);
			//emphasize green
			colors[2][i] = rgb(r * att, g, b * att);
			//emphasize yellow
			colors[3][i] = rgb(r, g, b * att);
			//emphasize blue
			colors[4][i] = rgb(r * att, g * att, b);
			//emphasize purple
			colors[5][i] = rgb(r, g * att, b);
			//emphasize cyan?
			colors[6][i] = rgb(r * att, g, b);
			//de-emph all 3 colors
			colors[7][i] = rgb(r * att, g * att, b * att);
		}
	...
	private static byte[][][] colorsToBytes(int[][] col)
	{
