...
public class PPU {
	...
	private final static boolean PPUDEBUG = PrefsSingleton.getInstance().getBoolean("ntView", false);
	...
	public final int read(final int regnum)
	{
		...
		case 4:
			...
			if (renderingOn() && (scanLineIdx <= 240)) {
				if (cycles < 64) {
					return 0xFF;
				} else if (cycles <= 256) {
					return 0x00;
		...
		case 7:
			...
			if (!renderingOn() || (scanLineIdx > 240 && scanLineIdx < (scanLineCount - 1))) {
				loopyV += vraminc;
			} else {
	 ...
	 */
	private final void runOneClockCycle()
	{
		...
		if (cycles == 1) {
			...
			if (scanLineIdx < 240) {
				bgcolors[scanLineIdx] = pal[0];
			}
		}
		if (scanLineIdx < 240 || scanLineIdx == (scanLineCount - 1)) {
			...
			if (scanLineIdx == (scanLineCount - 1)) {
		...
		} else if (scanLineIdx == vblankline && cycles == 1) {
		...
		if (div == 0) {
			mapper.cpu.runCycle(scanLineIdx, cycles);
			mapper.cpucycle(1);
			cpudividerctr = (cpudividerctr + 1) % cpudivider.length;
		}
		if (cycles == 257) {
			mapper.notifyscanline(scanLineIdx);
		} else if (cycles == 340) {
	...
	private void evalSprites()
	{
		...
		for (int spritestart = oamstart; spritestart < 255; spritestart += 4) {
			...
			offset = scanLineIdx - ypos;
			if (ypos > scanLineIdx || offset > (spritesize ? 15 : 7)) {
	...
	private int[] debugGetTile(final int offset)
	{
		...
		for (int i = 0; i < 8; ++i) {
			...
			for (int j = 0; j < 8; ++j) {
				//per pixel(1 bit)
				dat[8 * i + j] = ((((mapper.ppuRead(i + offset) & (utils.BIT7 - j)) != 0))
						? 0x555555 : 0)
	 ...
	 */
	public final void renderFrame(MainForm gui)
	{
		if (PPUDEBUG) {
			debugDraw();
		}
